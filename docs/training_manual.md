...
バージョン管理システムには大きく分けて「集中型」と「分散型」の2種類があり、Gitは**分散型（Distributed VCS）**です。

- **集中型（例：かつてのSVN等）**：履歴を持つ本体は中央サーバーに1つだけ存在し、各自の手元には「今の作業分」しかない
- **分散型（Git）**：中央のリモートリポジトリだけでなく、**clone した時点で全員の手元（ローカルリポジトリ）にも全履歴のコピーが存在する**

```mermaid
flowchart TB
    subgraph 集中型["集中型（例: SVN）"]
        direction TB
        CS["中央サーバー\n(全履歴を保持)"]
        CA["Aさんの手元\n(今の作業分のみ)"]
        CB["Bさんの手元\n(今の作業分のみ)"]
...
| 観点 | 集中型 | 分散型（Git） |
|---|---|---|
| 履歴の保持場所 | 中央サーバーのみ | 中央（リモート）＋全員の手元（ローカル）それぞれ |
| オフライン作業 | 中央サーバーに繋がらないと、過去履歴の閲覧やコミットができないことが多い | 手元に全履歴があるため、コミットや履歴確認はオフラインでも可能（pushする時だけネットが必要） |
| サーバー障害時 | 中央サーバーが壊れると履歴そのものを失うリスクがある | 誰か1人の手元にでも全履歴が残っていれば復元できる（冗長性が高い） |
| ブランチ作成の軽さ | サーバーとのやり取りが必要になりがちで比較的重い | ローカルだけで完結するため軽量。今回のように各自が気軽にブランチを切れるのはこの特性による |
| 学習コスト | 比較的シンプル | 「ローカル」と「リモート」を区別して理解する必要があり、最初は混乱しやすい（今回はここを丁寧に扱う） |

- 今回の研修で「clone」「push」「pull」という操作がわざわざ必要になるのは、Gitが分散型で「ローカル」と「リモート」がそれぞれ独立した履歴を持っているためです。集中型であればこの区別自体が存在しません
...
- ⚠️ **今回のリポジトリはPublic（公開）設定です。** インターネット上の誰からでも中身が見える状態なので、以下のようなものは絶対にコミット・pushしないでください
  - パスワード、APIキー、GCPの認証情報（サービスアカウントキー等）
  - 個人情報（実在する顧客の氏名・メールアドレス・電話番号など）。今回のダミーデータのような架空のものはOK
  - 会社の機密情報（非公開の契約内容、社外秘の数値など）
- 誤って機密情報を含むファイルをコミットしてしまった場合、**該当ファイルを削除するコミットを追加するだけでは不十分**です（Gitは履歴を残す仕組みのため、過去のコミットに情報が残り続けます）。もし誤ってpushしてしまった場合は、自己判断で対処せず、すぐにメンターに共有してください
- コミットメッセージにも、社外に見られて困る情報は書かないようにしましょう

**Gitの料金体系（参考）**

- **Git本体**：無料のオープンソースソフトウェアです。誰でも自由にインストールして使えます
- **GitHub**：個人アカウントであれば、Publicリポジトリ・Privateリポジトリともに基本機能は無料で利用できます（Freeプラン）。チーム機能や高度な管理機能が必要な場合は有料プラン（Team/Enterprise等）もありますが、今回の研修で使う範囲はすべて無料枠で完結します
- 今回のようにBigQueryなど外部サービスと連携する場合、料金が発生するのはGitHub側ではなく、GCP（BigQuery）側であることが多い点に注意してください
