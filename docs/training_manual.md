...
|---|---|---|---|
| 第1回 | 1時間 | 座学：Shell基礎 / Git基礎（clone）/ ミッション概要説明 | 座学後〜第2回までに：STEP1（clone・grepで課題確認）＋ SQLテスト本体（STEP2）を各自実施 |
| 第2回 | 1時間 | 座学：Gitブランチの概念 / PRの概念（前半） | 座学後〜第3回までに：STEP3（ブランチ作成・commit・push・PR作成）を各自実施 |
| 第3回 | 1時間 | 座学：PRレビュー文化（後半）＋ 提出済みPRの答え合わせ・レビュー | - |

> 💡 メンターへの補足：第2回の座学で「ブランチ」を教えるタイミングでは、受講者はまだブランチを切っていない状態を想定しています。座学で概念を理解させたうえで、宿題としてSTEP3（ブランチ作成〜PR作成）に取り組ませてください。
...
- [ ] **コードエディタのインストール**
  - 推奨：[Visual Studio Code](https://code.visualstudio.com/)（無料）
  - ファイルの中身を見たり、SQLファイルを編集したりする際に使用します。すでに使い慣れたエディタがあればそれで構いません
- [ ] **Gitのインストール確認**
  - ターミナルで `git --version` を実行し、バージョンが表示されればOK
  - 表示されない場合は、[Git公式サイト](https://git-scm.com/)からインストールしてください（Macは標準搭載されていることが多いです）
- [ ] **GitHubアカウントの作成**
  - [https://github.com/](https://github.com/) からサインアップ（未作成の場合）
  - 会社支給のメールアドレスで作成するか、既存アカウントを使うかは事前に寺島さんへ確認してください
  - 作成したアカウント名（GitHubユーザー名）を、リポジトリへの招待のためメンターに共有してください

### メンターから共有されるもの

以下がメンターから共有されているはずです。共有されていない場合は確認してください。

- [ ] GitHubリポジトリのURL（＋自分のアカウントがリポジトリへの書き込み権限を持っていること）
- [ ] GCPプロジェクトID
- [ ] BigQueryデータセット名（`training_dataset` 等）
- [ ] GCPアカウントへのアクセス権（BigQueryコンソールが開けるか事前に確認）
...
例えるなら、Kernelは実際に料理を作る「調理場」、Shellは注文を聞いて調理場に伝え、できあがった料理を運んでくる「ウェイター」のような関係です。ターミナルに`ls`と入力してから結果が表示されるまで、内部では次のようなやり取りが行われています。
...
第1回の座学が終わったら、次回（第2回）までの間に、各自のペースで以下を進めてください。
